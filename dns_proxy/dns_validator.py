...
class DNSValidator:
    ...
    def validate_dns_name(name: str) -> None:
        ...
        for label in labels:
            ...
            if label and not all(c.isalnum() or c == '-' for c in label):
                raise DNSValidationError(
                    f"Invalid characters in DNS label: '{label}'"
                )
            
            # Labels can't start or end with hyphen
            if label and (label.startswith('-') or label.endswith('-')):
                raise DNSValidationError(
                    f"DNS label cannot start or end with hyphen: '{label}'"
                )
    ...
    def validate_message(message: dns.Message) -> None:
        ...
        for query in message.queries:
            ...
            if query.type not in ALLOWED_QUERY_TYPES:
                raise DNSValidationError(
                    f"Unsupported query type: {query.type} for {query.name}"
                )
    ...
    @staticmethod
    def create_error_response(query_id: int, query: dns.Query, 
                            error_code: int = dns.EFORMAT) -> dns.Message:
        """
