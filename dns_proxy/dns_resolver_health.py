...
from .dns_resolver import DNSProxyResolver, CNAMEFlattener
from .config_human import HumanFriendlyConfig, UpstreamServer
from .health import HealthMonitor, ServerSelector, SelectionStrategy, QueryResult
...
class HealthAwareDNSResolver(DNSProxyResolver):
    ...
    def __init__(self, config: HumanFriendlyConfig, max_recursion: int = 10,
                 cache=None, remove_aaaa: bool = True,
        ...
        logger.info(f"Health-aware DNS resolver initialized with {len(self.upstream_configs)} servers")
    ...
    def _lookup(self, name, cls, type, timeout):
        """
        Override _lookup to use health-based server selection
        """
        ...
        logger.debug(f"Using {server_name} ({server_tuple[0]}:{server_tuple[1]}) for query: {name}")
        ...
        try:
            ...
            self.health_monitor.record_query_result(
                server_name, QueryResult.SUCCESS, response_time
            )
    ...
    # Add query method to match parent class interface
    def query(self, query, timeout=None):
        """Perform a DNS query using health-based server selection"""
        # Use parent class implementation which calls _lookup
