...
from .constants import DNS_DEFAULT_PORT, DNS_QUERY_TIMEOUT
...
class UpstreamServer:
    ...
    weight: int = 100           # 1-1000, higher = more traffic
...
class HumanFriendlyConfig(DNSProxyConfig):
    ...
    def _parse_upstream_section(self, section: str, seen_addresses: Dict[str, str]) -> UpstreamServer:
        ...
        if 'port' in options:
            try:
                ...
                raise HumanConfigError(
                    f"In [{section}]: Port must be a number between 1 and 65535",
                    f"Got '{options['port']}'"
                )
...
def migrate_legacy_config(config_path: str) -> str:
    ...
    if config.has_option('forwarder-dns', 'server-addresses'):
        ...
        for i, (addr, port) in enumerate(servers):
            ...
            used_names = [line.split(':')[1].rstrip(']') for line in output if line.startswith('[upstream:')]
