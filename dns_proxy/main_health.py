...
from .dns_resolver_health import HealthAwareDNSProtocol, HealthAwareDNSResolver
...
def main_with_health():
    ...
    try:
        ...
        if any(
            s.startswith("upstream:") or s.startswith("upstream.") for s in config.config.sections()
        ):
