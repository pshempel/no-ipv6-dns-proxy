...
class ServerMetrics:
    ...
    def record_query(self, result: QueryResult, response_time: Optional[float] = None):
        ...
        if result == QueryResult.SUCCESS:
            ...
            logger.debug(
                f"{self.server_name} query failed: {result.value} "
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            "average_response_time": (
                f"{self.average_response_time:.1f}ms" if self.average_response_time else "N/A"
            ),
            "median_response_time": (
                f"{self.median_response_time:.1f}ms" if self.median_response_time else "N/A"
            ),
            "health_score": f"{self.health_score * 100:.1f}%",
            "last_success": (
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(self.last_success_time))
                if self.last_success_time
                else "Never"
            ),
            "last_failure": (
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(self.last_failure_time))
