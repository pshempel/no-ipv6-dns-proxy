...
class LowestLatencySelector(ServerSelectorBase):
    ...
    def select(self, servers: List[ServerHealth]) -> Optional[ServerHealth]:
        ...
        sorted_servers = sorted(
            servers,
            key=lambda s: s.metrics.average_response_time or float('inf')
        )
...
class FailoverSelector(ServerSelectorBase):
    ...
    def select(self, servers: List[ServerHealth]) -> Optional[ServerHealth]:
        ...
        sorted_servers = sorted(
            servers,
            key=lambda s: (s.server.priority, s.server.name)
        )
...
class LeastQueriesSelector(ServerSelectorBase):
    ...
    def select(self, servers: List[ServerHealth]) -> Optional[ServerHealth]:
        ...
        sorted_servers = sorted(
            servers,
            key=lambda s: s.metrics.total_queries
        )
...
class ServerSelector:
        ...
        if not healthy_servers:
            ...
            if all_servers:
                ...
                sorted_by_health = sorted(
                    all_servers,
                    key=lambda s: s.metrics.health_score,
                    reverse=True
                )
    ...
    def select_multiple_servers(self, all_servers: List[ServerHealth], 
