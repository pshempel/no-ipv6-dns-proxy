...
# Context patterns to skip (like percentage calculations)
SKIP_CONTEXTS = [
    r"\* 100",  # Percentage calculations
    r"100\s*\*",  # Percentage calculations (reversed)
    ...
    for pattern, message in HARDCODED_PATTERNS:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if regex.search(code_part):
                ...
                skip = False
                for skip_pattern in SKIP_CONTEXTS:
                    if re.search(skip_pattern, code_part):
                        skip = True
                        break

                if not skip:
                    violations.append(
                        {"file": file_path, "line": i, "content": line.strip(), "message": message}
                    )
