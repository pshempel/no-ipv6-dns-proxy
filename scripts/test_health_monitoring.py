...
def test_dns_queries(resolver):
    ...
    for domain in test_domains:
        try:
            ...
            print(f"✅ {domain:20} -> {', '.join(ips[:2])}{'...' if len(ips) > 2 else ''} ({elapsed:.1f}ms)")
            
        except Exception as e:
            print(f"❌ {domain:20} -> Error: {e}")
        
        # Small delay between queries
        yield defer.Deferred().addCallback(lambda x: reactor.callLater(0.1, lambda: x.callback(None)))
...
def print_health_stats(resolver):
    ...
    for server_name, server_stats in stats.items():
        ...
        print(f"{server_name:<20} {status:<10} {server_stats['success_rate']:<10} "
...
def test_health_monitoring():
    ...
    try:
        ...
        yield defer.Deferred().addCallback(
            lambda x: reactor.callLater(3, lambda: x.callback(None))
        )
