...
class TestCacheBehavior(unittest.TestCase):
    ...
    def test_cache_cleanup_overhead(self):
        ...
        with patch('time.time', return_value=base_time + 2):
            ...
            self.assertGreaterEqual(remaining, 50, f"At least 50 non-expired entries should be accessible, found {remaining}")
    ...
    def test_cache_ttl_handling(self):
        ...
        response.answers = [
            dns.RRHeader(name='example.com', type=dns.A, ttl=3600)
        ]
        ...
        with patch('time.time', return_value=400):
            ...
            self.assertIsNotNone(self.cache.get(cache_key), "Entry should NOT expire after 300s (TTL is 3600)")
    ...
    def test_concurrent_cache_access(self):
        ...
        self.assertGreater(results['hits'], 50, f"Should have at least some cache hits, got {results['hits']}")
...
class TestCacheIntegration(unittest.TestCase):
    ...
    def test_duplicate_queries(self):
        ...
        mock_response.answers = [
            dns.RRHeader(name='example.com', type=dns.A, cls=dns.IN, 
                        ttl=300, payload=dns.Record_A('1.2.3.4'))
        ]
        ...
        self.assertEqual(1, self.resolver.resolver.lookupAddress.call_count,
