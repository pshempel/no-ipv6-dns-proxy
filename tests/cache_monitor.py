...
class CacheMonitor:
    ...
    def _monitored_set(self, key, data, ttl=None):
        ...
        self.query_log[key].append(
            {"time": datetime.now().isoformat(), "operation": "set", "ttl": ttl}
        )
    ...
    def analyze_patterns(self):
        ...
        analysis = {
            ...
            "repeated_misses": {},
            "hot_keys": {},
            "performance": {"avg_get_time_ms": 0, "max_get_time_ms": 0},
        }
    ...
    def print_live_stats(self):
        ...
        print(
            f"  Hit rate: {stats['hits']/(stats['hits']+stats['misses'])*100:.1f}%"
        ...
        if analysis["repeated_misses"]:
            ...
            for key, info in list(analysis["repeated_misses"].items())[:5]:
                print(
                    f"    {key}: {info['miss_count']} misses out of {info['total_accesses']} accesses"
                )
        ...
        if analysis["hot_keys"]:
            ...
            for key, info in list(analysis["hot_keys"].items())[:5]:
                print(
                    f"    {key}: {info['access_count']} accesses, {info['hit_rate']*100:.1f}% hit rate"
                )
...
def main():
    ...
    parser.add_argument(
        "--live", action="store_true", help="Run live monitoring (requires running DNS proxy)"
    )
    parser.add_argument("--test", action="store_true", help="Run test with real DNS queries")
    ...
    if args.test:
        ...
        result = subprocess.run(
            [sys.executable, os.path.join(os.path.dirname(__file__), "test_cache_behavior.py")],
            capture_output=True,
            text=True,
        )
        print(result.stdout)
