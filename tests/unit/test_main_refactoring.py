...
class TestMainRefactoring:
    ...
    def test_parse_arguments(self):
        ...
        test_args = ["--config", "/test/config.cfg", "--port", "5353", "--address", "127.0.0.1"]
                ...
                with patch("pwd.getpwnam", return_value=Mock(pw_uid=1000)):
                    with patch("grp.getgrnam", return_value=Mock(gr_gid=1000)):
                        with patch("os.chown") as mock_chown:
                            _handle_pid_file(
                                mock_config, mock_args, "dns-proxy", "dns-proxy", mock_logger
                            )
    ...
    def test_handle_daemonization(self):
        ...
        mock_args.daemonize = True
        with patch(
            "os.fork", side_effect=[0, 1]
        ):  # First fork returns 0 (child), second returns 1 (parent)
